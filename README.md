...
## 说说对事件循环的理解？
### 执行上下文
是JavaScript在被解析和运行时环境的抽象概念，执行上下文包括三个周期：创建，运行，销毁。共有三种类型：
#### 全局执行上下文
默认的执行上下文，即全局对象，浏览器环境中是 window，Node环境中是global
#### 函数执行上下文
每次调用函数时，都会为该函数创建一个新的执行上下文
#### Eval 函数执行上下文
运行在eval函数中的代码创建自己的执行上下文
### 执行上下文栈
存储函数调用的栈结构，遵循先进后出的原则
### 任务队列
- 任务分为同步任务和异步任务，所有的任务都在主线程上执行，形成一个执行栈
- 除了主线程之外，事件触发线程管理着一个任务队列，只要异步任务有了运行结果，就在任务队列之中放置一个事件
- 当执行栈中的所有同步任务执行完毕，系统就会读取任务队列，将对应的异步任务读取到执行栈中，开始执行
### 宏任务和微任务
#### 宏任务(macrotask)
script(整体代码)、setTimeout、setInterval、setImmediate(Node.js 环境)、UI 渲染、 I/O事件
#### 微任务(microtask)
Promise、MutaionObserver、process.nextTick(Node.js环境）
### 事件循环(Event Loop)
在事件循环中，每进行一次循环操作称为 tick，每一次tick的任务处理步骤如下：
- 执行当前执行栈中任务
- 执行完毕后，检查当前微任务队列中是否有微任务，如果存在则依次执行，直至清空微任务队列
- 检查渲染，然后浏览器GUI线程接管进行更新渲染
